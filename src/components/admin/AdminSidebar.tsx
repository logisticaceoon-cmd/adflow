        ...
        <div className="flex items-center justify-between px-5"
          ...
          <div className="flex items-center gap-2.5">
            <div className="w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0"
              style={{ background: 'linear-gradient(135deg, #22d3ee, #c5006a)' }}>
        ...
        <nav className="flex-1 px-3 py-4 flex flex-col gap-0.5 overflow-y-auto">
          ...
          {navItems.map(({ href, icon: Icon, label, exact }) => {
            ...
            return (
              <Link key={href} href={href}
                ...
                <Icon
                  size={16}
                  style={{ color: active ? '#ffffff' : '#22d3ee', opacity: active ? 1 : 0.7 }}
          ...
          <div className="mt-auto pt-4 flex flex-col gap-0.5">
            ...
            <Link href="/dashboard"
              ...
              style={{ color: '#6b8a8a' }}>
              <Eye size={16} strokeWidth={1.75} style={{ color: '#22d3ee', opacity: 0.5 }} />
              <span className="text-[13px] whitespace-nowrap">Ver panel usuario</span>
            </Link>
            <button onClick={handleLogout}
              className="nav-item nav-item-inactive text-left"
              style={{ color: '#6b8a8a' }}>
              <LogOut size={16} strokeWidth={1.75} style={{ color: '#22d3ee', opacity: 0.5 }} />
